...
Both sections use one shared design; there is no drawing-to-fret assignment. Each uses its calculated width and independently entered thickness. The shared profile supplies radius/width and corner-drop/thickness ratios. Thus `R_section = R_shared W_section / W_shared` and `D_section = D_shared T_section / T_shared`. Thickness is never inferred from width. You can adjust the shared profile in the workshop below the paired-template form.
